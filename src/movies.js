...
function scoresAverage(moviesArray) {
    if(!moviesArray.length){
        ...
        const sum = scores.reduce((prevValue, currValue) => prevValue + currValue, 0)
        const result = sum / scores.length
        return Math.round(result *100)/100
    }
}
scoresAverage(moviesCopy)
...
function dramaMoviesScore(moviesArray) {
    ...
    if(!dramaMovies.length){
        ...
        const sum = scores.reduce((prevValue, currValue) => prevValue + currValue)
        const result = sum / scores.length
        return Math.round(result*100)/100
    }
}
...
function orderAlphabetically(moviesArray) {
    const newArray = [...moviesArray]
    const sortAlph = newArray.sort((a, b) => {
        if(a.title.toLowerCase() < b.title.toLowerCase()){
            return -1
        }
        if(a.title.toLowerCase() > b.title.toLowerCase()){ 
    ...
    const movieTitles = sortAlph.map(movie => movie.title)
...
// orderAlphabetically(moviesCopy)
